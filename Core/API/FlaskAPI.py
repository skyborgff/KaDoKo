...
class FlaskAPI:
    def __init__(self, taskDict: dict):
        ...
        def library_list():
            ...
            info = self.tasker.addWaitReply('library_list')
            return jsonify(info)

        @self.app.route('/db_list', methods=['GET'])
        def db_list():
            # info = self.kadoko.ui.db_list()
            info = self.tasker.addWaitReply('db_list')
            return jsonify(info)
        ...
        def setup_settings():
            ...
            self.tasker.addTask('setup_settings', [post_data])
        ...
        def get_authentication_needed():
            ...
            info = self.tasker.addWaitReply('get_authentication_needed')
            return jsonify(info)
        ...
        def set_authentication():
            ...
            self.tasker.addTask('set_authentication', [post_data])
        ...
        def send_force(path):
            ...
            # print(self.taskDict)
            # print('Sending Graph')
            self.tasker.addWaitReply('generate_graph', ["Core/API/FlaskFiles/force/force.json"])
