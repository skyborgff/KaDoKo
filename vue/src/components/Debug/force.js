...
var svg = d3.select("svg")
  ...
  .call(d3.zoom().on("zoom", function () {
    svg.attr("transform", d3-event.event.transform)
  }))
  .append("g")
...
function dragstarted(d) {
  ...
  if (!d3-event.event.active) simulation.alphaTarget(0.01).restart();
  d.fx = d.x;
  d.fy = d.y;
}

function dragged(d) {
  d.fx = d3-event.event.x;
  d.fy = d3-event.event.y;
}

function dragended(d) {
  if (!d3-event.event.active) simulation.alphaTarget(0);
